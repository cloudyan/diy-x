...
此 bridge 文件放在 app 中，或构建在 H5 项目代码中

> WebViewJavascriptBridge 是 Webview 和 Html 交互通信的桥梁

```js
// utils/bridge/native.js
...
export default {
  created() {
    this.$bridge.ready(bridge => {
      bridge.getAppConfig(res => {
        console.log(res)
      })
    })
  },
